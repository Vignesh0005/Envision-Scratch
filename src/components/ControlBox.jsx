  ...
  const handleRecord = async () => {
    try {
      if (!isRecording) {
        ...
        console.log('🔍 Debug - Saved settings:', savedSettings);
        console.log('🔍 Debug - Parsed settings:', settings);
        console.log('🔍 Debug - Camera type:', cameraType);
        console.log('Starting camera with type:', cameraType);

        // Check if camera type is valid
        if (!cameraType) {
          console.error('❌ No camera type found in settings');
          alert('Please configure camera settings first');
          return;
        }
